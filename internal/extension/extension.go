// Package extension provides extension loading for FoxRay.
...
func hydrateVariables(s string, vars map[string]string) string {
	for key, val := range vars {
		s = strings.ReplaceAll(s, "${"+key+"}", jsonEscapeReplacement(val))
	}
	return s
}

func jsonEscapeReplacement(val string) string {
